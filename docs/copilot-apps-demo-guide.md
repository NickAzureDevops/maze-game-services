...
| [`NickAzureDevops/pac-man-services`](https://github.com/NickAzureDevops/pac-man-services) | Event consumer + dashboard | Backend event ingestion (`POST /event`) and live visualization (`GET /events` + polling UI). |

## Canvas role in the main repo

`pac-man-services` also includes a custom Copilot Canvas extension at `.github/extensions/pac-man-canvas`.
...
- a link to the live event stream in `pac-man-services`
...
Suggested role:
- expose repo-aware tools for `pac-man-game` + `pac-man-services`
...
1. Start `pac-man-services` on `http://localhost:3001`.
2. Start `pac-man-game`.
3. While playing the game, events are posted to `POST /event`.
4. `pac-man-services` stores events in memory and serves them via `GET /events`.
